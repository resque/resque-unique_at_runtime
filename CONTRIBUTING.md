...
- One spec file per class/module. For each class or module under `lib/`, keep all of its unit tests in a single spec file under `spec/` that mirrors the path and file name exactly: `lib/resque/unique_at_runtime/my_class.rb` -> `spec/resque/unique_at_runtime/my_class_spec.rb`.
...
[🧪build]: https://github.com/resque/resque-unique_at_runtime/actions
